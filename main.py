...
MY_CHANNEL = "@rostunnel"  # обновлено под твой запрос
...
def detect_exit_country_via_http(proxy_host: str) -> str:
    ...
    try:
        ...
        if r.status_code == 429:
            ...
            print("⚠️  ip-api вернул 429 — geo-API отключён")
            return "UNKNOWN"
...
def make_final_key(k_id, latency, country):
    """НОВЫЙ ФОРМАТ: {flag} LTE {номер} | тгк: @rostunnel"""
    flag = country_to_flag(country)
    key_number = k_id.split("/")[-1][-8:] if "/" in k_id else k_id[-8:]
    if not key_number.isalnum():
        key_number = str(abs(hash(k_id)) % 100000000).zfill(8)
    info_str = f"{flag} LTE {key_number} | тгк: @rostunnel"
    return f"{k_id}#{info_str}"

# ==================== Остальные функции (сохранение, чанки и т.д.) ====================
...
def save_fixed_chunks_euro(keys_list, folder):
    valid_keys = [k.strip() for k in keys_list if k and k.strip()]
    chunks = [valid_keys[i:i + EURO_CHUNK_LIMIT] for i in range(0, min(len(valid_keys), EURO_CHUNK_LIMIT * 3), EURO_CHUNK_LIMIT)]
    while len(chunks) < 3:
        chunks.append([])
    file_names = []
...
if __name__ == "__main__":
    print("=== CHECKER v6 (FAST/ALL + WHITE/BLACK + GEO-CACHE + THROTTLE + NEW LTE FORMAT) ===")
    load_ip_cache()
    ...
    save_ip_cache()
    save_json(HISTORY_FILE, {k: v for k, v in history.items() if current_time - v["time"] < MAX_HISTORY_AGE})
    ...
    print("\n" + "=" * 55)
    print("📊 ФИНАЛЬНЫЙ ОТЧЁТ — НОВЫЙ LTE ФОРМАТ АКТИВЕН!")
    print("=" * 55)
    print(f"RU FAST: {len(res_ru_fast)}, EURO FAST: {len(res_euro_fast)}")
    print("Ключи теперь в формате: 🇳🇱 LTE 83729104 | тгк: @rostunnel — чисто и мощно!")